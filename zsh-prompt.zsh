...
zstyle ':vcs_info:*' formats \
    '(%{%F{black}%K{green}%}%s%{%f%k%})-[%{%F{black}%K{blue}%}%b%{%f%k%}]'
zstyle ':vcs_info:*' actionformats \
    '(%{%F{black}%K{green}%}%s%{%f%k%})-[%{%F{black}%K{blue}%}%b%{%f%k%}|%{%F{white}%K{red}%}%a%{%f%k%}]'
...
prompt_bar_left_self="(%{%B%}%n%{%b%}%{%F{cyan}%}@%{%f%}%{%B%K{blue}%F{black}%}%m%{%b%})"
...
prompt_bar_right="-[%{%B%F{white}%K{magenta}%}%~%{%k%f%b%}]"
### 2行目左にでるプロンプト。
### %h: ヒストリ数。
