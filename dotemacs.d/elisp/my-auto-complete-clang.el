...
(defcustom my-ac-clang-executable
  (executable-find "clang")
  "*Location of clang executable"
  :group 'auto-complete
  :type 'file)

(defcustom my-ac-clang-auto-save nil
...
(defvar my-ac-clang-prefix-header nil)

;;; Set the Clang prefix header
(defun my-ac-clang-set-prefix-header (ph)
  (interactive
   (let ((def (car (directory-files "." t "\\([^.]h\\|[^h]\\).pch\\'" t))))
     (list
      (read-file-name (concat "Clang prefix header(current: " my-ac-clang-prefix-header ") : ")
                      (when def (file-name-directory def))
                      def nil (when def (file-name-nondirectory def))))))
  (cond ((string-match "^[\s\t]*$" ph)
         (setq my-ac-clang-prefix-header nil))
        (t
         (setq my-ac-clang-prefix-header ph))))

;;; Set a new cflags for clang
(defun my-ac-clang-set-cflags ()
  "set new cflags for clang from input string"
  (interactive)
  (setq my-ac-clang-flags (split-string (read-string "New cflags: "))))

;;; Set new cflags from shell command output
(defun my-ac-clang-set-cflags-from-shell-command ()
  "set new cflags for my-ac-clang from shell command output"
  (interactive)
...
(defconst my-ac-clang-completion-pattern
  "^COMPLETION: \\(%s[^\s\n:]*\\)\\(?: : \\)*\\(.*$\\)")

(defconst my-ac-clang-error-buffer-name "*clang error*")

(defun my-ac-clang-parse-output (prefix)
  (goto-char (point-min))
  (let ((pattern (format my-ac-clang-completion-pattern
                         (regexp-quote prefix)))
    ...
    (while (re-search-forward pattern nil t)
      ...
      (unless (string= "Pattern" match)
        ...
        (if (string= match prev-match)
            (progn
              (when detailed_info
                (setq match (propertize match
                                        'my-ac-clang-help
                                        (concat
                                         (get-text-property 0 'my-ac-clang-help (car lines))
                                         "\n"
                                         detailed_info)))
          ...
          (when detailed_info
            (setq match (propertize match 'my-ac-clang-help detailed_info)))
          (push match lines))))    
    lines))


(defun my-ac-clang-handle-error (res args)
  (goto-char (point-min))
  (let* ((buf (get-buffer-create my-ac-clang-error-buffer-name))
         (cmd (concat my-ac-clang-executable " " (mapconcat 'identity args " ")))
         (pattern (format my-ac-clang-completion-pattern ""))
...
(defun my-ac-clang-call-process (prefix &rest args)
  (let ((buf (get-buffer-create "*clang-output*"))
    ...
    (setq res (if my-ac-clang-auto-save
                  (apply 'call-process my-ac-clang-executable nil buf nil args)
                (apply 'call-process-region (point-min) (point-max)
                       my-ac-clang-executable nil buf nil args)))
    (with-current-buffer buf
      (unless (eq 0 res)
        (my-ac-clang-handle-error res args))
      ;; Still try to get any useful input.
      (my-ac-clang-parse-output prefix))))


(defsubst my-ac-clang-build-location (pos)
  (save-excursion
    (goto-char pos)
    (format "%s:%d:%d" (if my-ac-clang-auto-save buffer-file-name "-") (line-number-at-pos)
            (1+ (- (point) (line-beginning-position))))))

(defsubst my-ac-clang-lang-option ()
  (or (and my-ac-clang-lang-option-function
           (funcall my-ac-clang-lang-option-function))
...
(defsubst my-ac-clang-build-complete-args (pos)
  (append '("-cc1"
            "-fsyntax-only")
          (unless my-ac-clang-auto-save
            (list "-x" (my-ac-clang-lang-option)))
          my-ac-clang-flags
          (when (stringp my-ac-clang-prefix-header)
            (list "-include-pch" (expand-file-name my-ac-clang-prefix-header)))
          '("-code-completion-at")
          (list (my-ac-clang-build-location pos))
          (list (if my-ac-clang-auto-save buffer-file-name "-"))))


(defsubst my-ac-clang-clean-document (s)
  (when s
    (setq s (replace-regexp-in-string "<#\\|#>\\|\\[#" "" s))
    (setq s (replace-regexp-in-string "#\\]" " " s)))
  s)

(defun my-ac-clang-document (item)
  (if (stringp item)
      (let (s)
        (setq s (get-text-property 0 'my-ac-clang-help item))
        (my-ac-clang-clean-document s)))
  ;; (popup-item-property item 'my-ac-clang-help)
  )


(defface my-ac-clang-candidate-face
  '((t (:background "lightgray" :foreground "navy")))
  "Face for clang candidate"
  :group 'auto-complete)

(defface my-ac-clang-selection-face
...
(defun my-ac-clang-candidate ()
  (unless (ac-in-string/comment)
    (and my-ac-clang-auto-save
         (buffer-modified-p)
         (basic-save-buffer))
    (save-restriction
      (widen)
      (apply 'my-ac-clang-call-process
             ac-prefix
             (my-ac-clang-build-complete-args (- (point) (length ac-prefix)))))))


(defvar ac-template-start-point nil)
(defvar ac-template-candidates (list "ok" "no" "yes:)"))

(defun my-ac-clang-action ()
  (interactive)
  ;; (ac-last-quick-help)
  (let ((help (my-ac-clang-clean-document (get-text-property 0 'my-ac-clang-help (cdr ac-last-completion))))
        (raw-help (get-text-property 0 'my-ac-clang-help (cdr ac-last-completion)))
    ...
    (dolist (s ss)
      ...
      (cond ((string-match "^\\([^(]*\\)\\((.*)\\)" s)
             ...
             (push (propertize (my-ac-clang-clean-document args) 'my-ac-clang-help ret-t
            ...
            ((string-match "^\\([^(]*\\)(\\*)\\((.*)\\)" ret-t) ;; check whether it is a function ptr
             ...
             (push (propertize args 'my-ac-clang-help ret-f 'raw-args "") candidates)
             (when (string-match ", \\.\\.\\." args)
               (setq args (replace-regexp-in-string ", \\.\\.\\." "" args))
               (push (propertize args 'my-ac-clang-help ret-f 'raw-args "") candidates)))))
    (cond (candidates
...
(ac-define-source my-clang
  '((candidates . my-ac-clang-candidate)
    (candidate-face . my-ac-clang-candidate-face)
    (selection-face . my-ac-clang-selection-face)
    (prefix . my-ac-clang-prefix)
    (requires . 0)
    (document . my-ac-clang-document)
    (action . my-ac-clang-action)
    (cache)
    (symbol . "c")))

(defun my-ac-clang-same-count-in-string (c1 c2 s)
...
(defun my-ac-clang-split-args (s)
           ...
           (let ((res (list)) (pre "") subs)
             (while sl
               ...
               (unless (string= pre "")
                 ...
                 (setq pre ""))
               (cond ((and (my-ac-clang-same-count-in-string ?\< ?\> subs)
                           (my-ac-clang-same-count-in-string ?\( ?\) subs))
                      (push subs res))
                     (t
                      (setq pre subs))))
             (nreverse res)))
  ...
  (unless (null ac-template-start-point)
    (let ((pos (point)) sl (snp "")
      ...
      (cond ((string= s "")
             ...
             (setq s (replace-regexp-in-string "^(\\|)$" "" s))
             (setq sl (my-ac-clang-split-args s))
...
(provide 'my-auto-complete-clang)
